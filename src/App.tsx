...
import { useDraggable, useKonami, useResizable, useTerminalSize } from "./hook";
...
const App = () => {
	...
	const terminalSize = useTerminalSize(terminalRef);
		...
		if (e.key === "Enter") {
			...
			if (output === "__CLEAR__") {
				...
				setCommandHistory((prev) => [
					...prev,
					{ command: "history", output: historyOutput },
				]);
			} else {
			...
			<div className="terminal-container crt-flicker">
				...
				<div className="h-full w-full flex flex-col items-center justify-center relative z-10">
					...
					>
						...
						>
							...
							<TerminalLine delay={TIMING.catOutput}>
								<div className="command-output text-[#888] leading-relaxed">
									...
									<br />I specialize in scalable systems - from microservices
						...
						<div
							className={`terminal-footer ${isMinimized ? "terminal-footer-minimized" : ""}`}
						>
							<span>PID: 1337</span>
							<span className="usage-bar">
								...
								<span className="usage-pct">3.1%</span>
							</span>
							<span className="usage-bar">
								<span className="usage-label">MEM</span>[
								<span className="bar-fill">||||</span>
								<span className="bar-empty">------</span>]
								<span className="usage-pct">42%</span>
							</span>
							<span>
								{footerTime.toLocaleDateString("en-US", {
									weekday: "short",
									month: "short",
									day: "numeric",
								})}{" "}
								{footerTime.toLocaleTimeString("en-US", {
									hour: "2-digit",
									minute: "2-digit",
									second: "2-digit",
								})}
					...
					<div className="text-center mt-8 text-[#333] text-xs">
						...
						<span className="text-[#ff3333]">{"<3"}</span> using React +
						TypeScript
					</div>
				</div>
			</div>
